...
const MEDICONNECT_API = process.env.MEDICONNECT_API_URL || 'http://localhost:5000';
const HOSPITAL_A_API = process.env.HOSPITAL_A_API_URL || 'http://localhost:8001';

async function testRejectWebhook() {
    try {
        console.log('Fetching removal requests from MediConnect...');
        const res = await axios.get(`${MEDICONNECT_API}/api/removal-requests`);
        ...
        if (!pendingReq) {
            ...
            const shareRes = await axios.post(`${HOSPITAL_A_API}/api/removal-requests`, {
            ...
            const hospReqList = await axios.get(`${HOSPITAL_A_API}/api/removal-requests`);
            ...
            if (hospReq) {
                ...
                await axios.put(`${HOSPITAL_A_API}/api/removal-requests/${hospReq._id}/approve`);
        ...
        const rejectRes = await axios.put(`${MEDICONNECT_API}/api/removal-requests/${pendingReq._id}/reject`);
        ...
        const hospReqsRes = await axios.get(`${HOSPITAL_A_API}/api/removal-requests`);
